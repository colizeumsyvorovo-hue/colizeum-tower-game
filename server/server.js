    ...
    try {
      ...
      if (bot) {
        ...
        // Отправляем уведомление если достигли 100 бонусов (только один раз)
          ...
          await bot.telegram.sendMessage(
            user.telegram_id,
            `❄️ <b>ЗИМНИЙ ПОДЪЁМ - ДОСТИЖЕНИЕ!</b> ❄️\n\n` +
            `🎯 <b>"Поднимайся выше - собирай больше бонусов!"</b>\n\n` +
            `🎉 Поздравляем! Вы накопили <b>${bonusesAfter} бонусов</b> в банке!\n\n` +
            `💪 Отличная работа! Вы на пути к вершине!\n\n` +
            `⏳ Осталось накопить: <b>${remaining} бонусов</b> до вывода\n\n` +
            `🎁 <b>Напоминание о выводе бонусов:</b>\n` +
            `1️⃣ Накопите <b>500 бонусов</b> в банке\n` +
            `2️⃣ Пополните игровой баланс на <b>50% от суммы</b> (250 рублей)\n` +
            `3️⃣ Подойдите к ресепшну в одном из клубов:\n` +
            `   • Суворова 27а\n` +
            `   • Ленина 26\n` +
            `4️⃣ Получите свои 500 бонусов!\n\n` +
            `💡 <b>Совет:</b> Играйте каждый день за бонусы, чтобы быстрее накопить нужную сумму!\n\n` +
            `🚀 Продолжайте играть и поднимайтесь выше!`,
            {
              parse_mode: 'HTML'
            }
          );
        }
        // Отправляем уведомление если достигли 500 бонусов (только один раз)
        else if ((gameType === 'normal' || gameType === 'bonus') && bonusesBefore < 500 && bonusesAfter >= 500) {
          await bot.telegram.sendMessage(
            user.telegram_id,
